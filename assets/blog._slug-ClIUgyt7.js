import{G as f,z as b,r as j,t as k,a as m,j as e,L as n,p as l,q as N,I as g,P as u,g as w,J as S}from"./index-Ch_UTdnQ.js";import{p as T,D as L}from"./Diagrams-DOEt8cuD.js";import{N as q}from"./NewsletterSignup-9UL2uGTQ.js";function p(t){return t.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"")}function h(t){return new Date(t).toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"})}g.use({renderer:{checkbox({checked:t}){return`<input ${t?'checked="" ':""}disabled="" type="checkbox" aria-hidden="true" tabindex="-1"> `},heading({tokens:t,depth:a}){const s=this.parser.parseInline(t),r=p(t.map(o=>o.raw??"").join(""));return`<h${a} id="${r}"${a===2?' data-rz-reveal class="rz-section-head"':""}>${s}</h${a}>
`}}});function C(t){const a=g.parse(I(t),{async:!1});return e.jsx("div",{dangerouslySetInnerHTML:{__html:a}})}const A=new Set(m.map(t=>t.slug));function I(t){return t.replace(/\[([^\]]+)\]\(\/blog\/([^/#?)]+)\/?\)/g,(a,s,r)=>A.has(r)?a:s)}function z(t){return t.split(`
`).filter(a=>/^##\s+/.test(a)).map(a=>{const s=a.replace(/^##\s+/,"").trim();return{id:p(s),text:s}})}const _=`(() => {
