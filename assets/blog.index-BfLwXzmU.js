import{b as q,Y as L,N as g,x as N,y as i,z as e,L as h,V as y}from"./index-CNSzerP1.js";const E=["Network product","PSP / orchestration","Bank / regulated fintech","Founder / operator"],w=["Visa","Mastercard","Stripe","Adyen","Wise","Thunes","DLocal"];function S(a){const s=`${a.title} ${a.tags.join(" ")} ${a.content??""}`.toLowerCase();return w.filter(r=>s.includes(r.toLowerCase()))}function k(a){return new Date(a).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"})}function b(a){const s=a.tags.map(l=>l.toLowerCase()).join(" "),r=new Set;return/network|scheme|visa|mastercard|tokeniz|acceptance/.test(s+" "+a.category.toLowerCase())&&r.add("Network product"),/orchestration|api|hosted|gateway|psp|local payment/.test(s)&&r.add("PSP / orchestration"),/swift|iso 20022|gpi|correspondent|aml|sanctions|pci|iso 27001|regulated/.test(s)&&r.add("Bank / regulated fintech"),r.size===0&&r.add("Founder / operator"),[...r]}function j(a,s=!1){return{"data-blog-result":"","data-blog-featured":s?"true":"false","data-search":`${a.title} ${a.description} ${a.tags.join(" ")}`.toLowerCase(),"data-hub":N(a)?.slug??"","data-readers":b(a).join("|"),"data-companies":S(a).join("|")}}const C=`
(() => {
