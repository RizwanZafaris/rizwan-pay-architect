...
Related: [Reconciliation Is Product Infrastructure](/blog/reconciliation-is-product-infrastructure) · [Financial Controls Are Product Requirements](/blog/financial-controls-are-product-requirements)`,"agentic-payments-operations-works-vs-theatre":`Every vendor pitch in 2025 has "agentic" somewhere on the slide. Most of the demos are theatre — a scripted scenario in which the agent makes the exact decision the demo was rehearsed around, in a context that does not exist in production. The product-manager audience nods politely. The buyer signs anyway because the board is asking what the company's AI agent story is.

Behind the marketing, a real productive subset exists. There are 5–6 operations workflows in payments where an LLM-driven agent — given the right tools, the right guard rails, and the right operating frame — earns its keep. There are 4–5 more where the demo is theatre and the production deployment ranges from useless to dangerous.

This is the honest operator read on the difference. Where agents work, what makes them work, where they fail, and what makes them fail.

## A short definition

An "agent" in this context is an LLM-driven workflow that:

- Takes a goal expressed in natural language
- Has access to a set of tools (APIs, databases, internal systems)
- Plans a sequence of actions to achieve the goal
- Executes the actions
- Observes the results and adjusts

Compare this to a non-agentic LLM call: single input, single output, no tools, no plan, no iteration. The "agentic" claim adds the planning loop, the tool use, and the iterative execution.

In payments operations, the question is whether the planning loop produces better outcomes than a deterministic workflow would. Sometimes yes; sometimes no; the distinction is in the workflow's properties.

## Where agentic ops genuinely earn their keep

### 1. Dispute case enrichment

**The workflow.** A chargeback lands. The agent gathers evidence: pulls the original transaction, the cardholder's prior transaction history, the merchant's product catalogue, the device fingerprint, the IP geolocation, the delivery confirmation if any, the cardholder's communications. It assembles a case file. It drafts the representment language.

**Why agents fit.** The work is *evidence gathering across multiple systems* in a partially-templated structure. Each chargeback is slightly different (different reason code, different merchant, different prior-transaction shape). The agent's planning loop is the right fit because the next step depends on what the previous step found.

**The lift.** The case-investigator's per-case time drops from 25-40 minutes to 5-10 minutes of review. The agent does not make the final decision; the investigator confirms and submits.

**The guard rail.** The agent never *submits* the representment. It produces the draft and the evidence pack; a human investigator confirms.

### 2. AML alert triage

**The workflow.** An AML alert fires (sanctions match, unusual transaction pattern, structuring suspicion). The agent gathers the entity profile, prior transaction patterns, public-record information, news mentions, related entities. It triages the alert: clean false positive (e.g., name collision with a high-confidence non-match), needs investigator review, or needs escalation to compliance.

**Why agents fit.** Triage is a multi-source synthesis task. The decision logic is "given these facts, which bucket?" — a classification problem with structured-output discipline that LLMs handle competently.

**The lift.** Tier-1 analyst throughput roughly doubles. Clean false positives are auto-dismissed (with audit log); real cases get to investigators with the context already gathered.

**The guard rail.** The agent never *dismisses* a sanctions hit — only flags as "appears to be name collision, suggest dismissal". Human confirms. The audit trail of agent + human decision is regulator-facing.

### 3. Merchant-support level-1 triage

**The workflow.** A merchant raises a support ticket. The agent reads the ticket, queries the merchant's recent transactions, account status, settlement history, recent alerts. It produces either an auto-response (for FAQ-class tickets where the agent has high confidence) or a triage note for the support analyst with the relevant context pre-pulled.

**Why agents fit.** Support ticketing is a high-volume routing problem with merchant-context lookup. The agent's tool use (reading the merchant's actual state, not just the ticket text) produces better triage than text-only classification.

**The lift.** Tier-1 throughput doubles or triples on the auto-response slice. Tier-2 analyst time per case drops because the context is pre-pulled.

**The guard rail.** Auto-response only on tickets where the agent's confidence exceeds a calibrated threshold AND the category is on the "auto-respond safe" list. Everything else routes to humans.

### 4. Settlement-break investigation

**The workflow.** A reconciliation break appears. The agent pulls the affected transaction, the merchant's recent settlement history, the scheme settlement file, the bank statement entry (or absence thereof). It produces a diagnostic: most likely cause, evidence supporting that diagnosis, recommended fix.

**Why agents fit.** Settlement-break investigation is a structured detective workflow — pull data, compare, hypothesise, validate. The agent does the pulling and comparing; the human reviews and decides on the fix.

**The lift.** Investigator time per break drops from 15-30 minutes to 3-5 minutes of review and decision.

**The guard rail.** The agent never *fixes* the break (does not write to the ledger). It produces the diagnostic; a senior reconciliation analyst applies the fix.

### 5. Onboarding document follow-up

**The workflow.** A KYB submission is missing a document or has a low-confidence extraction. The agent identifies what is missing or unclear, drafts a follow-up message to the merchant explaining what is needed in their language, in a tone appropriate to the platform.

**Why agents fit.** The work is a structured-output writing task with context (which document is missing, what merchant tier they're in, what tone the platform uses). Templated follow-ups are too rigid; case-by-case human writing is too slow.

**The lift.** Onboarding ops time per follow-up drops from 8-15 minutes to under a minute (review and send). Cycle time on document re-requests drops.

**The guard rail.** The follow-up message is a draft until a KYB analyst reviews. Auto-send is reserved for the very narrow case of a single missing standard document with no ambiguity.

## Where the agentic demo is theatre

### 1. "Agent decides whether to approve a transaction"

The demo is impressive: cardholder pays, agent considers fraud signals, agent makes a decision in 200ms. In production, this is a fraud-decision rule engine plus an LLM wrapper that adds nothing — the rule engine was already real-time, the LLM adds latency without improving accuracy, and the regulator wants to know exactly how the decision was made. Real fraud decisioning is classical ML + rules; agents do not belong on the auth path.

### 2. "Agent processes the entire chargeback end-to-end"

A subset of demos show the agent reviewing, gathering, *and submitting* the chargeback response. In production, no compliance team will allow the agent to submit on its own — the legal liability and the regulator-facing audit posture are too sensitive. The agent's role is enrichment and drafting; submission is human.

### 3. "Agent autonomously onboards a merchant"

The demo shows the agent extracting documents, screening, adjudicating, and approving. In production, the screening engine is separate, the adjudication is human, and the approval has formal sign-off. The agent does extraction + first-pass routing; the rest is not agent work.

### 4. "Agent writes the new product feature"

A class of demo shows the agent reading a PRD, drafting code, deploying to production. In a regulated platform, the engineering review, the code review, the test coverage, the deployment governance, and the rollback discipline make the "agent ships code" demo theatre. The agent might draft code; the engineering team ships it.

### 5. "Agent manages the regulator relationship"

The most dangerous theatre. Regulators have explicit guidance that automated systems making consequential decisions must have human accountability. An agent that drafts regulator submissions, attends meetings (via assistants), or makes representations is not a productive workflow — it is a compliance incident in waiting.

## The architectural pattern that separates real from theatre

Five properties of agent workflows that actually ship in payments:

**1. The agent has a tightly scoped goal.** "Enrich this chargeback case" is scoped. "Handle the disputes function" is not. Agents that work have one goal per workflow, not a goal hierarchy.

**2. The agent uses tools, not just text.** Agents that only generate text are not agents in the sense that matters — they are LLM calls. The productive agents are integrated with the platform's actual systems (read the transaction, read the merchant profile, read the screening result).

**3. The agent's output is a draft, not a decision.** The productive pattern is: the agent produces a recommendation with evidence; a human confirms. The exceptions are narrow (high-confidence auto-dismissal of clear false-positive sanctions hits, auto-responses to clearly-templated support tickets) and audited.

**4. The agent's decisions are auditable per-step.** Every tool call, every intermediate output, every reasoning step is logged. The regulator inquiry "show me how the agent decided" is answerable with a query.

**5. The human-in-the-loop is permanent.** Throughput improves; reviewer count does not drop to zero. The pattern is "humans review more cases per hour" not "humans are out of the loop". The platforms that ship the latter ship regulator findings.

## What to ask vendors

A working set of questions to separate productive agent workflows from theatre during a vendor evaluation:

- **What is the goal scope of the agent?** If it includes more than one operational function, it is too broad.
- **What tools does the agent have access to?** If the list is generic ("internet search, code execution"), it is not designed for your platform.
- **What is the human-in-the-loop pattern?** If the answer is "agent makes the decision; human reviews if escalated", the threshold for escalation is doing the work.
- **What is the audit posture?** If the vendor cannot show a regulator-facing audit log of agent decisions, the platform's compliance function will reject the deployment.
- **What is the failure mode?** When the agent gets it wrong, who is liable and what is the recovery path?
- **Show me a real customer in production for >6 months.** Demos are theatre. Six months of production data separates the use cases that work from the ones that drift.

The vendors that have shipped real agentic ops can answer all six concretely. The vendors selling theatre cannot.

## The senior-PM tell

The interview question that distinguishes senior PMs on agentic AI: "your CEO wants us to deploy an agent for X. Walk me through how you decide if it is real or theatre."

The junior answer talks about ROI estimates. The senior answer reads: what is the scope; what are the tools; what is the failure mode; what is the human-in-the-loop posture; what is the audit pattern; can a regulator inquiry on agent decisions be satisfied in 30 minutes. If all six answers line up, ship the pilot. If two or three are weak, deploy with extra guard rails and reduced scope. If four or more are weak, kill it.

That answer is the operating posture. It is also the answer that prevents the AI programme from becoming the next year's regulator finding.

## FAQ

**Are these the only productive agentic workflows in payments?**
The five above are the most consistently-productive that I have seen across multiple platforms. Other niches exist (treasury reconciliation enrichment, vendor-management ticketing, internal-audit pre-prep) but the pattern is the same: scoped goal, tool use, draft output, human confirmation, auditable trail.

**Will agents take over more of these workflows over time?**
Slowly. The bottleneck is not the agent capability; it is the regulator posture and the platform's appetite for liability shift. As model performance improves and as regulators issue clearer guidance, the human-in-the-loop threshold will move — but slowly, and asymmetrically per jurisdiction.

**What is the right vendor vs. build call?**
For workflow-specific agents (dispute enrichment, AML triage, onboarding follow-up), specialised vendors are typically ahead of in-house builds. For platform-deep workflows (settlement-break investigation, merchant-support triage that requires deep internal-system access), in-house builds usually win because the tool-integration matters more than the LLM choice.

**How do agents interact with the seven ML-failure patterns?**
Pattern overlap. Concept drift applies (agent prompts decay as systems change). Label leakage applies (training the agent on outcomes that include human-review changes). Ops integration applies (the agent's output has to land cleanly in the reviewer's workflow). All seven patterns from the [Why AI/ML Solutions Fail in Production Payments](/blog/why-ai-ml-solutions-fail-production-payments) essay apply to agentic systems too.

**What's the cost?**
Per-action LLM cost is real but rarely the binding constraint. The binding constraint is the integration and audit engineering: the tool integrations, the audit logging, the human-review tooling, the failure-mode handling. A productive agentic workflow takes ~3-6 engineering months of integration work to ship cleanly; the ongoing LLM cost is small in comparison.

**How is this different from RPA?**
RPA (robotic process automation) is rigid — it executes the same screen-clicks every time. Agents handle variation: different chargeback shapes, different document classes, different merchant contexts. RPA works for fully-deterministic workflows; agents work for partially-deterministic ones. Many platforms use both — RPA for the fixed parts, agents for the variable parts.

---

If this resonated, also read [AI Auto-Escalation for Payment Ops](/blog/ai-auto-escalation-payment-ops), [KYB Document Extraction: A Realistic LLM Use Case](/blog/kyb-document-extraction-realistic-llm-use-case), and [Why AI / ML Solutions Fail in Production Payments](/blog/why-ai-ml-solutions-fail-production-payments).`,"bin-routing-scheme-selection-override-default":`For most of the card-acquiring stack, the scheme is set the moment the cardholder hands over the card: a Visa BIN goes to Visa, a Mastercard BIN goes to Mastercard, the acquirer's job is to forward the message. That single sentence has been the default for two decades, and it is wrong in roughly 40% of real card portfolios — co-badged cards, debit cards in regulated routing markets, local-scheme dual-rail debit, and merchant-selected scheme-of-preference programmes all give the acquirer a genuine choice, and the choice is worth real auth-rate and interchange basis points.
...
**3. Local-scheme override on domestic traffic.** Many markets have a national debit scheme that *must* be offered as an option (Mada in Saudi, NPCI/RuPay in India, BCEL/POS in Vietnam, Mir in Russia, PROS in the Philippines, Verve in Nigeria). Even when the cardholder presents a Visa or Mastercard, regulator pressure may push the acquirer to surface a local-rail option.
...
A simple framing. The default rail (the card-brand logo on the front of the card) optimises for the *scheme's* economics: the international brand wants the international rail. The override rail (whichever co-badge or local rail is also available) often optimises for the *merchant's* and *acquirer's* economics.
...
The implication: CE3.0 is a *programme*, not a feature. It needs to be co-ordinated with a Mastercard equivalent programme, a non-fraud disputes programme, and the card-present surface. Treating CE3.0 in isolation produces a portal that wins one quarter of disputes brilliantly and ignores the other three quarters.
...
What does *not* change: the accounting, the audit, the tax, the regulator reporting, the FX-policy compliance, the fraud risk profile, the operational discipline. The treasurer's job is the same job; some of the tools are different.
...
Use them as the open conversation in 60-minute slots, not as multi-choice scoring items. The point is not to grade. The point is to see how the candidate *thinks* about the things that decide whether a payments PM ships or sinks.
...
**What it tests.** Diagnostic posture under ambiguity. The right answer is a *method*, not a list of fixes.
...
**Watch for.** Whether the candidate names the *evidence* that turned the call. Senior PMs reverse on data, not vibes.
...
Resist the temptation to grade each question 1–5. The signal in a senior PM interview is *cumulative*: a candidate who answers eight of twelve well at the senior level is hireable; a candidate who answers eleven well at the mid-level is not.
...
The twelve are about *operating posture* in payments. The other surfaces matter; they get their own rounds.
...
A feature is a *model-ready signal* derived from one or more raw data sources. Examples:
...
Most production credit-scoring systems use *families* of models, not a single model:
...
If this resonated, also read [Why AI/ML Solutions Fail in Production Payments](/blog/why-ai-ml-solutions-fail-production-payments), [KYB Document Extraction: A Realistic LLM Use Case](/blog/kyb-document-extraction-realistic-llm-use-case), and [Where ML Beats AI: Six Payment Problems an LLM Cannot Touch](/blog/where-ml-beats-ai-payment-problems-llm-cant-touch).`,"kyb-document-extraction-realistic-llm-use-case":`Most LLM use cases proposed in fintech the last two years have been theatre. Generative summaries of dashboards. Chatbots that answer FAQs the support team had already solved. "AI co-pilots" that produce well-formatted output and zero operational lift. Every payments executive has been on the receiving end of the vendor pitch and the engineering enthusiasm both, and both have largely failed to ship anything that earned its keep.

KYB document extraction is the rare exception. The use case is narrow enough to define cleanly, structured enough that LLM output is verifiable, common enough that the volume justifies the build, and ops-painful enough that automation produces real time savings. The teams that have shipped it cleanly — with the guard rails, the audit posture, and the human-in-the-loop discipline — have cut KYB cycle times from 7–14 days to 1–3 days without lowering the standard.

This is the operator's view of the use case: the seven document classes that matter, the pipeline architecture, the four guard rails that prevent regulator findings, and the realistic ROI a senior PM can promise.

## What KYB document extraction actually involves

Know Your Business (KYB) is the merchant-side counterpart of KYC. Before an acquirer or payment processor onboards a business merchant, regulators (and schemes) require evidence:

- The legal entity exists and is registered.
- The beneficial owners (people holding >25% / >10% / threshold-varies-by-market) are known and screened.
- The business is in a permitted category for the platform's licence.
- Sanctions and PEP screening pass for the entity, the directors, and the UBOs.
- The business's claimed nature of operation matches the documentary evidence.

The documentary evidence arrives as a stack of files: certificates of incorporation, articles of association, beneficial-ownership declarations, government-issued IDs for directors, utility bills as address proof, bank statements as financial proof, regulatory licences for licensed categories. A KYB operator reads each document, extracts the relevant fields, and produces a structured record the screening pipeline can act on.

This is the work LLMs can genuinely accelerate. The documents are unstructured (free-form PDFs, scanned images, government forms with varying layouts). The fields to extract are structured (legal name, registration number, registered address, directors, shareholders, beneficial-owner thresholds). The output is checkable (the screening pipeline runs, the operator reviews edge cases, audit logs all decisions).

The use case fits the LLM's strengths and avoids its weaknesses. The team that ships it well treats it as a *production operations system*, not as an AI experiment.

## The seven document classes that matter

A working KYB document taxonomy. Most jurisdictions have analogous documents under different names; the structure generalises.

**1. Certificate of incorporation / commercial registration.** The primary proof of legal entity existence. UAE: trade licence. KSA: commercial registration. UK: Certificate of Incorporation. US: state-level articles of incorporation. Issuing authority, registration number, registered date, registered address, legal name.

**2. Memorandum / articles of association / shareholder register.** Names directors, named shareholders, share split, business purpose. The document that produces the initial UBO map.

**3. Beneficial-ownership declarations.** Where the UBOs are not visible in the shareholder register (often because the holding company structure obscures them), a separate declaration is required. Multi-page, free-form, often the highest-effort document to parse.

**4. Director ID documents.** Passports, national IDs, residence cards. Extract name, ID number, date of birth, nationality, expiry — match against the director list from the articles.

**5. Proof of address (entity and directors).** Utility bill, bank statement, tenancy. Extract address, issue date, validity (typically must be ≤3 months old).

**6. Sector-specific licence.** Healthcare, gambling, financial services, food. Issuing authority, licence number, scope of operation, validity period.

**7. Recent bank statements.** Some platforms require these as proof of operating account; some require them for AML transaction-pattern checks. Account holder name, IBAN / account number, statement period.

A senior PM ships the extraction pipeline class-by-class, not "extract everything from anything". Each class has its own prompt, its own validation, and its own human-review SLA.

## The pipeline architecture

A working production architecture, end to end:

1. **Ingestion.** Document upload via merchant portal or KYB operator interface. Files normalised to PDF; OCR'd if needed.

2. **Classification.** A lightweight classifier (CNN or rule-based) determines which of the seven classes the document is. Wrong classification produces wrong extraction; this step is gated on confidence — low confidence routes to a human classifier.

3. **Structured extraction.** Per-class LLM prompt extracts the structured fields. The prompt is *not* "extract everything"; it is "extract the following fields, return JSON in this schema, return null if a field is not present, return your confidence per field."

4. **Validation.** Programmatic checks on the LLM output: legal-name format, registration-number format per issuing country, date validity, cross-document consistency (director name in passport matches director name in articles).

5. **Confidence scoring.** Per-field confidence from the LLM, plus programmatic validation results, combined into a per-document and per-record confidence score.

6. **Routing.** High-confidence records flow to automated screening. Low-confidence records route to human review. Borderline cases route to a senior reviewer.

7. **Screening pipeline.** Sanctions, PEP, adverse media — runs on the extracted record. Hits route to compliance for adjudication.

8. **Audit log.** Every extraction event, every confidence score, every human review, every screening decision is logged with the input document, the LLM prompt version, the extraction output, and the reviewer's decision. The audit log is regulator-facing artefact.

The pipeline pattern is: LLM does the heavy lifting on structure-from-unstructured; programmatic validation catches the LLM's failure modes; humans handle edge cases; audit logs everything.

## The four guard rails

A KYB pipeline without these four produces a regulator finding within a year. The team that ships them at design time avoids the finding.

**Guard rail 1 — Field-level confidence + automatic human review threshold.**

The LLM produces a confidence per field. The pipeline auto-rejects extraction with low overall confidence. The threshold is tuned per field class — registration numbers need higher confidence than registered addresses (because a wrong registration number is a clean fail, a wrong address triggers proof-of-address re-request anyway).

A rule of thumb: any field at <85% confidence triggers human review. Any *document* with two or more fields below 85% triggers full re-review. The thresholds are calibrated weekly against actual reviewer disagreement rates.

**Guard rail 2 — Cross-document consistency checks.**

A director named "Mohammed Hassan" in the articles of association and "Muhammad Hasan" in the passport is a routine consistency issue (Arabic-to-Latin transliteration). The pipeline flags name variants for review; it does not auto-pass them.

A registered address that differs by one digit between the trade licence and the proof of address is either a typo or a fraud signal. The pipeline routes the discrepancy to human review with both documents side-by-side.

**Guard rail 3 — Audit-evidenced regulator posture.**

Every extraction is logged with the document, the prompt version, the LLM output, the validation results, the human review decision (if applicable), and the final record. The log is queryable per merchant, per onboarding event, per regulator-requested cohort. If a regulator asks "show me your KYB evidence for this merchant", the artefacts are one query away.

The team that treats the audit log as a write-only system that nobody queries until the regulator asks is the team that discovers, the day the regulator asks, that the log is incomplete or unparseable.

**Guard rail 4 — Foundation-model lock-in awareness.**

The pipeline depends on a foundation model (GPT-4, Claude, Gemini, or whichever). The model is a vendor dependency. Three considerations:

- The model can be deprecated or version-bumped by the vendor. The pipeline must be portable across foundation models — same prompts, different backend.
- The model's behaviour can change between versions. Re-running the extraction on the same document with a newer model produces different output. The pipeline pins the model version per onboarding event.
- The vendor's terms of use evolve. Data residency, model fine-tuning rights, audit access — all are vendor-specific. The senior PM tracks the vendor terms as part of the model governance.

## What the human-in-the-loop discipline looks like

The single most common LLM-extraction failure mode is treating "high confidence" as "no human review needed". The team that ships well treats human review as a permanent operating function, not as a fallback for edge cases.

A working model:

- **Tier 1 reviewers.** Junior KYB analysts who handle 80% of automated cases that flagged for review — typically simple discrepancies, low-confidence single fields, sanctions hits that look like name collisions.
- **Tier 2 reviewers.** Senior KYB analysts who handle the 20% of cases tier 1 escalates — complex UBO structures, jurisdiction-specific issues, sanctions hits that are non-trivially named.
- **Compliance adjudication.** For sanctions hits or PEP matches that pass review, the case lands with compliance for adjudication. Not an analyst decision.

The throughput maths:

- Pre-LLM: a senior KYB analyst processes ~3–5 merchants per day.
- Post-LLM pipeline: a tier-1 analyst processes ~15–25 merchants per day (mostly auto-extracted, reviewing only the flagged cases).
- Tier-2 and compliance throughput is unchanged — those cases were going to require senior review regardless.

The total cycle-time compression is real (7–14 days to 1–3 days median); the labour saved is real (~4–5x throughput on tier-1 work). The savings do not come from replacing humans; they come from removing tier-1 work that was previously done by tier-2 or senior analysts.

## What this does *not* solve

Three areas where LLM extraction does not help:

**1. UBO discovery beyond the documents.** When the holding structure is multi-layered and the UBO is not declared on any document the merchant submitted, the LLM cannot find them. Corporate-registry lookups, transparency-register integrations, and human investigative work remain.

**2. Sanctions screening itself.** The LLM extracts the structured record; the screening engine (a separate, specialised system) runs the matching against sanctions lists, PEP lists, adverse media. Conflating extraction with screening is a common architectural error.

**3. Adverse media interpretation.** A press article about the director from five years ago — is that disqualifying, contextual, or noise? This is human judgment supported by media-monitoring tools, not LLM output.

The senior PM scopes the LLM use case narrowly — extraction only — and integrates with the existing screening and adjudication infrastructure. Scope creep ("the LLM can also screen, also adjudicate, also write the SAR") is how this use case turns from earning its keep to producing regulator findings.

## The realistic ROI

A working back-of-envelope for a platform onboarding 200 merchants per month:

- **Time saved per merchant**: ~80 minutes of analyst time (extraction + initial review)
- **Monthly time saved**: 200 × 80 minutes = 270 hours
- **Annualised time saved**: ~3,200 hours, or roughly 2 full-time tier-1 analyst equivalents
- **Time-to-onboard compression**: from 7-14 days to 1-3 days median
- **Merchant satisfaction lift**: meaningful — the faster onboarding cycle is the merchant-side win

Investment: one senior PM, one engineer, one MLOps engineer for one quarter; ongoing one MLOps engineer + one product analyst part-time. Pays back in ~6-9 months on a 200-merchant-per-month platform.

The auditable second-order benefit is the audit log itself — KYB regulator inquiries shift from "find the file, build the case" multi-day exercises to "run the query" minutes-long exercises.

## The senior-PM tell

The interview question that distinguishes senior PMs on this use case: "your KYB pipeline goes live. The first regulator inquiry asks for the evidence chain on merchant X. Walk me through what you produce."

The junior answer talks about the LLM output. The senior answer produces: the original documents as ingested, the classification confidence per document, the extraction prompt version per document, the LLM output per document, the validation results, the human-review decision with reviewer ID and timestamp, the cross-document consistency checks, the final screening record, the screening engine's matching result, the compliance adjudication if any.

Every artefact is timestamped, queryable, immutable. The regulator inquiry takes 20 minutes to satisfy.

That answer is the difference between a programme that ships and a programme that produces the audit observation.

## FAQ

**Can a smaller open-source LLM do this work?**
For routine document classes (commercial registrations in major jurisdictions, standard ID documents), yes — open-source models with structured-output prompting and good validation do well. For complex UBO declarations and sector-specific licences, the larger frontier models still win on out-of-the-box accuracy. Most production deployments mix both.

**Should the platform fine-tune a model on its own KYB data?**
Usually not, for two reasons. The dataset is small (KYB documents are not millions of examples). The benefit is marginal compared to better prompts + better validation + better human review. Fine-tuning becomes worth it only at very large scale or for highly idiosyncratic document classes.

**What about hallucination?**
Real. The extraction prompt explicitly instructs "return null if a field is not present"; the validation pipeline checks that fields the LLM claims exist actually appear in the document text. The combination of structured extraction + validation + human review handles hallucination as a routine failure mode.

**Does the pipeline work for KYC (consumer onboarding) too?**
Partially. KYC has fewer document classes and tighter regulatory expectations on consumer data processing. The same architecture applies; the field set is smaller; the consent and data-processing posture is stricter. Many platforms ship KYB first because the regulatory bar is shaped for business processing and the per-merchant value is higher.

**What about real-time extraction at the merchant portal?**
A common ask. The pipeline supports it (extraction in seconds), but the screening + adjudication path runs in minutes-to-hours. Merchant-portal UX should show extraction in real time, screening as "in progress", and the final decision when adjudication completes.

**Who owns this — risk, ops, product, or engineering?**
Joint, with product as the integrator. Risk owns the model governance and the regulator-facing audit posture. Engineering owns the pipeline and the MLOps. Ops owns the human-review function. Product owns the merchant-facing experience and the cross-functional design.

---

If this resonated, also read [KYB Automation Without Blowing Up Risk](/blog/kyb-automation-without-blowing-up-risk), [Why AI / ML Solutions Fail in Production Payments](/blog/why-ai-ml-solutions-fail-production-payments), and [Where ML Beats AI: Six Payment Problems an LLM Cannot Touch](/blog/where-ml-beats-ai-payment-problems-llm-cant-touch).`,"mastercard-send-visa-direct-push-payments":`For most of card-rail history, the cards were the receive-only side of the network. Money came in (cardholder pushes funds to merchant); money did not go out. The two scheme push-payment products — Visa Direct and Mastercard Send — are what changed that. Both let a regulated payer push funds *to* a card credential at the receiving end, in something close to real time, through the same global card network that the cardholder uses for everyday purchases.
...
**3. The cross-border posture is asymmetric.** Each market is dominantly a remittance *destination* (inbound), with outbound restricted by foreign-exchange controls. Cross-border operators that build for outbound corridors discover the controls late; those that build for inbound have a clearer path.
...
If this resonated, also read [Product Management for Payments Platforms](/blog/product-management-for-payments-platforms), [Hiring Fintech PMs: 12 Interview Questions](/blog/hiring-fintech-pms-twelve-interview-questions), and [CSPO + RICE in Practice](/blog/cspo-rice-payments-roadmap-walkthrough).`,"payments-prd-template-nine-sections-senior-pm":`The SaaS PRD survived a decade of changing tools because its structure was almost universal: problem, goal, users, stories, scope, success metric, open questions. Eight headings, one page in good cases, three pages in long ones. The shape worked because the failure modes of a SaaS feature are mostly UX failures and the maths is mostly conversion.

A payments PRD shaped like that ships incidents.

The reason is not that payments PMs need to be more rigorous than SaaS PMs. It is that the failure modes of a payments feature live in places a SaaS PRD has no section for. Money movement timing. Scheme rules. Reconciliation symmetry. Audit posture. The PRD has to force the writer to think about each of those before code starts, because they are not catchable in QA.

This is the template that holds, after writing or reviewing more than a hundred payments PRDs across acquirer, processor, wallet, BNPL, cross-border, and KYB products. Nine sections. The shape is not negotiable. The depth of each section flexes with the size of the change.

## Section 1 — Problem and context

Half a page. Two paragraphs.

The first paragraph names the problem in operator language, not feature language. "We want to add Apple Pay" is feature language; not a problem. "Mobile auth rate on iOS Safari is 6 points below desktop, and merchant ops attribute most of the lost authorisations to manual card-entry abandonment" is the problem. The PRD lives or dies in how this paragraph is written, because every later decision derives from it.

The second paragraph is the strategic context. Why now, not last quarter, not next. What changed externally (regulator deadline, scheme mandate, large merchant pressure, competitor move) or internally (capacity, prior work landed, telemetry signal). If the answer to "why now" is "we have time", deprioritise the PRD; it is not yet a problem worth shipping.

**Prompt that forces clarity:** "If I do not ship this in the next two quarters, what specifically gets worse?"

## Section 2 — Goal and non-goals

One short paragraph plus two bullet lists.

The goal is a single sentence in product-outcome language, not feature language. "Add a 3DS2 step-up exemption flow" is feature; not a goal. "Lift iOS Safari auth rate to within 2 points of desktop without increasing the fraud rate" is a goal. The goal has a numeric target and a guard-rail.

Non-goals are equally important. The reason most payments PRDs spawn rework is that the engineering team interpreted scope wider than the PM intended. List the things this PRD is explicitly not solving — even when the team might want to. Non-goals are the future PRDs in the queue.

**Prompt that forces clarity:** "What is the simplest possible thing that would close this problem, and what am I explicitly choosing not to do beyond that?"

## Section 3 — Money movement and timing

This is the section that does not exist in SaaS PRDs and where most payments PRDs fail.

For every flow the feature touches, the PRD walks through the money path:

- **Who pays whom.** Cardholder → issuer → scheme → acquirer → merchant settlement bank → merchant. Name each leg even if it is obvious.
- **When the money moves.** Authorisation hold (T+0), capture (T+0 or T+n), settlement (T+1 / T+2 / same-day), refund timing, chargeback timing.
- **What the failure recovery looks like.** If the issuer auth succeeds but the capture fails, what happens? If the scheme settles but the merchant bank rejects, what happens? Name the recovery path for each.
- **Who carries the float.** During the gap between authorisation and settlement, whose balance sheet is the money on?
- **What is reversible and what is not.** Authorisations can be voided; captures cannot be voided after settlement; settled funds reverse only through dispute or refund. The PRD says which states the feature touches.

If this section is empty in your PRD, the feature has not been designed yet, regardless of how complete the rest of the document looks.

**Prompt that forces clarity:** "Draw the money path on a whiteboard with timing labels. If you cannot draw it from memory, you do not understand the feature yet."

## Section 4 — Regulator and scheme posture

Every payments feature lives inside a regulatory and scheme envelope. The PRD names which licences and which scheme rulebooks the feature touches, and what each requires.

For regulator posture:

- Which licence(s) cover this activity in which markets
- Which articles or directives apply (PSD2 SCA, AMLD6, GDPR, market-specific central-bank circulars)
- What customer disclosures are required
- What records must be kept and for how long
- What audit evidence must be produced and where it lives

For scheme posture:

- Which scheme rules apply (Visa Core Rules, Mastercard Standards, scheme-equivalents for local rails)
- Whether certification is required and what the certification path is
- Whether the feature changes the BIN-routing logic, interchange category, or merchant category code
- Whether the feature triggers a scheme attestation event

This section is the artefact that the legal and compliance review uses. If it is missing or thin, the review takes three rounds instead of one. If it is complete, the review is often a one-pass approval.

**Prompt that forces clarity:** "If a regulator or scheme auditor walked in tomorrow and asked 'show me your design evidence for this control', could you hand them this section as-is?"

## Section 5 — Reconciliation and ledger impact

For every state transition the feature introduces, the PRD names the ledger entries and the reconciliation pairings.

- **Internal ledger entries.** What account is debited, what is credited, with what reference, at what timestamp.
- **External statement pairings.** Which scheme settlement file row, which bank statement entry, which correspondent message, will this transaction land in. The PRD names the field the matching engine will key on (UETR, ARN, scheme transaction ID, reference).
- **Reconciliation breaks.** Which states could leave a ledger entry without a paired external row and how the ops team detects and resolves the break.
- **End-of-day posture.** What the daily reconciliation report shows for this flow. Whether a break in this flow blocks settlement or is logged for next-day review.

Most payments incidents are ledger incidents discovered hours or days after the bad transaction. The PRD that names the reconciliation pairings before code prevents the incident; the PRD that does not, ships it.

**Prompt that forces clarity:** "If a regional finance team called at 9pm asking 'why is the ledger ten thousand dirhams off today,' can the on-call engineer trace the answer through the artefacts this PRD names?"

## Section 6 — Failure modes and edge cases

Every payments PRD has a section listing what can go wrong and what the product does when it does.

Group failures into four categories:

**Upstream failures.** Scheme timeouts, issuer declines, gateway errors, network partitions. For each, the PRD specifies the retry policy, the merchant-facing error message, and the consumer-facing message.

**Downstream failures.** Settlement bank rejection, sponsor bank delays, correspondent issues, payout file failure. For each, the PRD specifies what state the transaction is left in and how the merchant is notified.

**Adversarial cases.** Fraud-triggered declines, scheme risk holds, regulator-mandated freezes, AML alerts. For each, the PRD specifies the customer experience, the case-management workflow, and the SLA for resolution.

**Race conditions.** Concurrent capture and refund, dispute opened while still settling, void after partial capture, refund before settlement. For each, the PRD specifies the precedence and the resulting ledger state.

If the team cannot fill this section, the team has not yet thought through what they are building. Edge cases are the substance of payments product work; the happy path is the easy half.

**Prompt that forces clarity:** "What is the worst outcome a customer can experience because of this feature, and what specifically prevents it?"

## Section 7 — Telemetry, KPIs, and acceptance criteria

The metric the goal is written in is in here. So is every metric the team will watch on the way to that goal, and the acceptance criteria that mark the work as "done".

Three layers:

**Primary KPIs.** The numeric targets the PRD commits to. Authorisation rate at X%, settlement timing at Y, dispute cycle time at Z. Each KPI has a baseline reading, a target, and a measurement window.

**Guard-rail KPIs.** The metrics that must not move adversely. Fraud rate, refund-to-payment ratio, support contact rate, reconciliation break count. Each guard-rail has a threshold beyond which the rollout is paused.

**Telemetry events.** The instrumentation the engineering team builds to observe the feature in production. Event names, properties, downstream queries. The PRD names these so the engineering team does not invent them later from memory.

Acceptance criteria are written in past tense, observable, and checkable by the PO (per the CSPO discipline):

- "The 3DS2 step-up flow has been tested with the four most common issuer combinations and has produced a successful auth in each."
- "The new dispute portal has surfaced at least 95% of in-flight cases with correct status to ten reference merchants."
- "The settlement reconciliation engine has zero breaks on the first three days of the new flow."

**Prompt that forces clarity:** "Could a junior PM, given only this section, decide whether the feature has shipped or has not?"

## Section 8 — Rollout plan and reversibility

Payments features do not ship by feature flag flip. The PRD specifies the rollout shape.

- **Stage 1: internal traffic.** Synthetic transactions, dogfood accounts, sandbox merchants.
- **Stage 2: reference merchants.** Three to ten merchants chosen for their willingness to absorb early bugs and their representativeness of the broader portfolio.
- **Stage 3: progressive rollout.** Percentage of merchants or percentage of traffic, with explicit gates between stages (KPI thresholds met, guard-rails not tripped, reconciliation clean).
- **Stage 4: GA.** The point at which the legacy flow is deprecated, with a timeline for deprecation.

Reversibility is the part most teams forget. For every stage, the PRD names what "rollback" looks like and at which point rollback stops being possible without merchant-facing consequences. If the feature changes settlement timing, the rollback after settlement has occurred is not a rollback — it is a remediation, with its own runbook.

**Prompt that forces clarity:** "At each stage, what does it look like to stop, and what does it look like to undo what has already happened?"

## Section 9 — Open questions and decisions

Every payments PRD has an open-questions section because at the moment of writing, not everything is known.

Each open question has:

- A clear question
- A decision owner
- A decision deadline (date, not "ASAP")
- The constraint the answer must satisfy

Each decided question — the ones already closed before the PRD ships to engineering — has:

- The decision in one sentence
- Who made it
- Why it was made (the relevant constraint)
- What the alternatives were

The "decisions log" is the most-read section of a payments PRD six months after launch. It is the artefact the next PM (or the audit team, or the post-incident review) uses to understand why the system behaves the way it does. Write it as if the next reader knows nothing about the room you were in when you wrote it.

**Prompt that forces clarity:** "If the writer of this PRD left the company tomorrow, could the next PM continue without a verbal handover?"

## What is not in the template

A few sections that other PRD templates carry, that this one does not:

- **User personas.** Payments products have constituencies (Section 2 of the [Product Management for Payments Platforms](/blog/product-management-for-payments-platforms) essay), not personas. Personas leak SaaS habits into payments thinking.
- **Wireframes.** Wireframes belong in design artefacts, linked from Section 7, not pasted into the PRD. The PRD is a decision document, not a design surface.
- **Engineering implementation.** Implementation choices belong in engineering design docs that descend from the PRD. The PRD names the constraints, not the design.
- **Pricing model.** Pricing lives in a separate commercial PRD, even when the product change has revenue implications. Mixing them produces document-shaped compromises.

## How long should a payments PRD be?

A working answer: long enough that the nine sections are each substantive, and short enough that the engineering lead reads it without skipping. In practice, four to nine pages for a meaningful payments feature; one to two pages for a small change that still touches money or audit. If a PRD is over fifteen pages, it is either a strategy document pretending to be a PRD, or it should be split into two PRDs.

## The senior-PM tell

The interview question that catches juniors out is not "do you write PRDs?" It is "what is in a payments PRD that is not in a SaaS PRD, and why?". The junior answer adds compliance bullets to a SaaS template. The senior answer reaches for the money-movement section, the reconciliation section, the scheme-posture section, and the open-decisions log — and explains why the SaaS template is structurally incomplete, not just under-detailed.

That answer is the difference between a PM who will ship working payment features and one who will ship incidents.

## FAQ

**Is this template too heavy for small changes?**
For very small changes (one config flip, one copy change, one telemetry-event addition), use a one-page mini-PRD that still has the nine section headings but with most of them written as one sentence. The discipline is in the section structure; the depth flexes.

**Does this template work for non-card payment products?**
Yes. The nine sections were designed for any product that moves money — card acquiring, A2A rails, cross-border, BNPL, wallets, virtual accounts. The "scheme posture" section is generalised to "rule envelope" for products without a card scheme.

**Should the PRD live in Confluence, Notion, Google Docs, or a markdown file in the repo?**
Whichever the engineering team reads. The platform matters less than the convention that the PRD is the single source of truth and that decisions land in the PRD rather than in chat.

**Who writes each section?**
The PM owns the PRD. Sections 4 (regulator and scheme) and 5 (reconciliation) usually require input from compliance and finance respectively, but the PM still owns the final wording. Owning the wording is the test of whether the PM understands the constraint.

**When does the PRD freeze?**
Never fully. The acceptance criteria and goal freeze before engineering starts; the open-questions section keeps changing; the decisions log keeps growing. A PRD is alive for the lifetime of the feature, not just the build.

**How is the PRD different from a design doc?**
The PRD is the product decision; the design doc is the engineering choice. The PRD names the constraints; the design doc names the implementation. One PRD typically maps to one or more design docs.

---

If this resonated, you might also want to read [CSPO + RICE in Practice: A Real Payments Roadmap Walkthrough](/blog/cspo-rice-payments-roadmap-walkthrough), [Product Management for Payments Platforms](/blog/product-management-for-payments-platforms), or [Program Management vs Product Management in Fintech](/blog/program-vs-product-management-fintech).`,"pmo-maturity-model-five-stages":`The canonical PMO maturity models — OPM3, P3M3, Gartner's PMO maturity matrix — all share the same shape: five levels, each with a tidy set of attributes, presented as a checklist the organisation works through in order. The shape is unobjectionable. It is also useless for actually deciding what a fintech PMO should do next.

The problem is not the levels themselves; it is the implied progression. The standard models suggest that the right move from level 2 is level 3, then level 4, then level 5. In a fintech (or any organisation with non-trivial regulatory exposure), the right move is often *backwards* — pushing back on a Director who wants a level-4 governance overlay when the org is genuinely at level 2 saves more value than building the level-4 toolkit.

This is the operator's version: five stages of PMO maturity drawn from running and observing fintech PMOs across companies from one-person operations through 30-person regulated programmes. The honest signs of each stage, the typical move from one to the next, the four traps that lock a PMO at the wrong level, and the cases where staying at a lower level is the right call.

## The five stages

### Stage 1 — Ad-hoc

**What it looks like.** There is no PMO. Delivery happens through individual project managers attached to specific squads, programmes, or initiatives. Status reporting is per-project, in whatever format the PM chose. Cross-project coordination happens because two PMs noticed they had a dependency and Slack-DMed each other. RAID logs are personal artefacts in the PM's Notion or Confluence.

**Honest signs.** Executives have to ask three people to get a status on any cross-functional initiative. There is no centralised view of the company's portfolio of work. The same risk shows up in three separate post-mortems before anyone notices it is structural.

**When this is the right level.** Startups under ~50 people, where the founders can hold the portfolio view in their heads and direct decisions in real time. Pretending you have a PMO when you don't is overhead without lift.

**The move from here.** Hire (or appoint) a single Head of PMO who owns the central view. Stop short of building a full team; you need a person, not a department.

### Stage 2 — Centralised

**What it looks like.** A Head of PMO exists, owns the central portfolio view, runs a weekly status across the company's active initiatives. There may be 1-3 PMO analysts producing reports, maintaining the RAID log, and chasing programme PMs for status updates.

**Honest signs.** Executives know what is shipping in the next quarter (the PMO publishes the slate). Risk and dependency conversations happen at the central forum, not in individual PM DMs. The same risk does not show up in three post-mortems anymore — it shows up in the RAID log and gets discussed once.

**When this is the right level.** 50–300 people, before regulatory programmes dominate. Most early-stage fintechs land here and stay until either growth (more programmes) or regulation (formal governance expectations) forces the next step.

**The move from here.** When regulators or large customers start asking for evidence of programme governance, or when the portfolio of work exceeds 6-8 simultaneous large programmes that the PMO head can personally track, scale to the next stage.

### Stage 3 — Governed

**What it looks like.** The PMO has 5-15 people. There is a formal RAID + SteerCo discipline: every major programme has a SteerCo cadence, RAID logs are maintained in a shared toolkit, escalation paths are documented. Decision rights are explicit. There is a portfolio review at the executive level on a monthly or quarterly cadence.

**Honest signs.** A regulator inquiry on "show me your governance for programme X" is answerable in a day. New programmes start with a charter, not a slack message. Risk owners are named per RAID line.

**When this is the right level.** Fintechs at 300-1,500 people with multiple regulated programmes, scheme certifications, or cross-functional delivery as a routine operating pattern. The vast majority of mature mid-size fintechs land here.

**The move from here.** Most fintechs should not move beyond stage 3 until they have specific reasons (scale, regulatory complexity, cross-jurisdiction operations) that require it. The trap (see below) is to move "up" the maturity ladder because the model suggests it, not because the organisation needs it.

### Stage 4 — Strategic

**What it looks like.** The PMO has 15-30+ people. The function is not just delivery governance; it is strategic — running benefits-realisation tracking, portfolio prioritisation analytics, capacity modelling, vendor governance. The Head of PMO is in the C-suite or one reporting line away. The PMO partners with strategy and finance on annual planning.

**Honest signs.** Strategic planning runs through the PMO's portfolio model. Programme prioritisation conversations cite PMO-produced capacity and dependency analysis. Vendor relationships are governed centrally with PMO-owned playbooks.

**When this is the right level.** Large fintechs (>1,500 people), banks, or fintechs operating across multiple regulated jurisdictions with high portfolio complexity. The strategic PMO produces decision-grade analysis that the executive team relies on.

**The move from here.** Toward stage 5 only when the organisation's scale and risk complexity genuinely demands it — usually banks, large insurance groups, or major payment infrastructure operators.

### Stage 5 — Embedded

**What it looks like.** The PMO is no longer a separate function in the traditional sense. PMO discipline is embedded across business lines — every BU has its own PMO function that ties into a thin central PMO that handles cross-BU coordination, group-wide reporting, and group-wide programme governance. The PMO leadership is distributed.

**Honest signs.** PMO discipline is something every senior leader practices, not something a separate team imposes. RAID, SteerCo, and benefits-realisation are part of the operating system, not artefacts produced for governance.

**When this is the right level.** Very few fintechs. Major banks, large insurers, regulated infrastructure operators. The complexity that justifies this level is rare outside the largest financial-services organisations.

## The four traps that lock PMOs at the wrong level

### Trap 1 — Premature scaling

A 200-person fintech hires a 12-person PMO because the new Head of PMO came from a bank where 12-person PMOs were normal. The org is at stage 2 in reality but operating at stage 3 in headcount. Result: the PMO produces governance theatre that the rest of the org views as overhead. Reports get filed and nobody reads them. The PMO becomes a target in the next cost-cutting cycle.

**The fix.** Match the PMO size to the stage the org actually needs, not the stage the PMO head is used to. A stage-2 fintech is well-served by a Head of PMO + 1-2 analysts.

### Trap 2 — Stuck on artefacts

The PMO produces the RAID log, the SteerCo deck, the status report. The artefacts exist. Nobody reads them. The PMO measures itself on the quality of the artefacts; the organisation does not benefit. The PMO is at stage-3 maturity by artefact and stage-2 by impact.

**The fix.** Measure the PMO on outcomes — programmes that shipped on time, regulatory deadlines met, dependencies caught before they cost a quarter. If the artefacts are not changing decisions, scrap them and start with the questions the executive team is actually asking.

### Trap 3 — Scaling up when the answer is scaling down

A stage-3 PMO encounters strategic complexity (multi-country, multi-regulator). The instinct is to scale to stage 4 — build the strategic analytics team, the portfolio prioritisation model, the capacity planning function. Sometimes the right move is the opposite: spin off PMO functions into the BUs (the stage-5 pattern) earlier than the maturity model would suggest, because central governance is becoming a bottleneck.

**The fix.** Ask whether the bottleneck is "we lack central visibility" (move up the ladder) or "central visibility is now a bottleneck" (distribute). The answer is BU-specific; the maturity model presumes the first answer.

### Trap 4 — Confusing scale with maturity

A 30-person PMO at a stage-2 organisation is not a stage-4 PMO; it is an over-staffed stage-2 PMO. Conversely, a 3-person PMO at a stage-3 organisation is not under-staffed — if the discipline is right, the size is right.

**The fix.** Maturity is about the operating model, not the headcount. The honest sign of stage 3 is "regulator inquiry on programme X is answerable in a day"; that capability is reachable with 3-15 people depending on the organisation's complexity.

## How to diagnose the stage you are at

Five questions. Answer each one honestly. The cluster of answers points to the stage.

**1. If a regulator asked tomorrow for the governance evidence on your three largest programmes, how long would it take to produce?**
- Hours: stage 4+
- One day: stage 3
- One week: stage 2
- Multiple weeks of scrambling: stage 1

**2. Where does the company's portfolio view live?**
- Embedded in business lines + a central group view: stage 5
- In the PMO's strategic analytics: stage 4
- In a central RAID + SteerCo system: stage 3
- In the PMO head's head: stage 2
- Nowhere; ask three people: stage 1

**3. How does the executive team prioritise the quarter?**
- PMO-produced capacity + dependency analysis informs decisions: stage 4+
- Programme-by-programme review with PMO-produced status: stage 3
- Ad-hoc with PMO-produced status as input: stage 2
- Direct executive conversation with no PMO input: stage 1

**4. How does a new programme start?**
- Embedded BU process with central registration: stage 5
- Charter + PMO programme establishment process: stage 3-4
- Slack message + PM assignment: stage 1-2

**5. Who owns programme risk?**
- Named owner per risk + PMO-tracked + benefits-realised review: stage 4
- Named owner per risk + PMO-tracked: stage 3
- Programme PM tracks, escalates ad-hoc: stage 2
- Nobody clearly: stage 1

The cluster of answers usually lands within one stage. If the answers are split across multiple stages (some at 4, some at 2), the PMO is mismatched — over-investing in one dimension and under-investing in another.

## What "moving up a stage" actually requires

Three things, in roughly this order:

**1. Sponsorship.** A senior executive (COO, CPO, or CEO) sponsors the PMO maturity transition. Without sponsorship, the PMO function builds artefacts that the org never adopts. The sponsorship has to be active — not "we support the PMO" but "I, as COO, expect the executive team to engage with the PMO outputs".

**2. Talent.** Each stage needs the right talent. Stage 1 → 2 needs a Head of PMO who is comfortable being a department of one. Stage 2 → 3 needs PMO analysts who can run RAID/SteerCo across multiple programmes. Stage 3 → 4 needs strategic analytics talent — closer to consulting than to project management. Hiring a stage-4 talent profile into a stage-2 org produces frustration on both sides.

**3. Operating-model change.** Each stage requires a shift in how the rest of the organisation operates. Stage 1 → 2 requires programme leads to send status to the PMO. Stage 2 → 3 requires executives to attend SteerCos and act on RAID escalations. Stage 3 → 4 requires the executive team to use the PMO's portfolio analytics in their decisions. Without the operating-model change, the PMO maturity transition is cosmetic.

The transition is real when all three are in place. It is theatre when any one is missing.

## The senior-PMO tell

The interview question that distinguishes senior PMO leaders: "your CEO wants to move the PMO from stage 2 to stage 4 in nine months because the board is asking. What do you say?"

The junior answer says yes and starts building. The senior answer reads: the move requires sponsorship, talent, and operating-model change; we can build the PMO function in nine months but the operating-model change takes 18-24; we can ship the artefacts but they will not be useful until the executive team's behaviour changes; here is a 9-month plan to ship the stage-3 capability we genuinely need, and a 24-month plan to mature into stage 4 if the operating model evolves to match. We should not promise stage-4 in nine months because shipping stage-4 artefacts without operating-model change produces governance theatre, which the board will read as PMO failure even though the artefacts exist.

That conversation is the difference between a PMO that ships maturity and a PMO that ships PowerPoints.

## FAQ

**What if the executive team genuinely wants stage 4 in nine months?**
Push back through the discipline above. If after that they still want stage 4 in nine months, accept the brief and document the operating-model dependencies in writing — so that when the artefacts are not being used in nine months, the conversation is about why the operating model did not change, not about why the PMO did not deliver.

**Should every PMO aim for stage 5?**
No. Stage 5 is appropriate for large banks and very large insurers. Most fintechs should aim for stage 3 and stay there, evolving toward stage 4 only when scale and regulatory complexity demand it.

**Is there a stage 0?**
Sometimes — organisations where individual squads run their own delivery without any cross-squad coordination. The move to stage 1 (ad-hoc cross-squad coordination, no formal PMO) is straightforward.

**How does the PMO maturity relate to the PgM maturity?**
PgM (program management) is the role; PMO is the function. A high-PgM-maturity organisation can sit at stage 2 of PMO maturity for years if the individual PgMs are strong enough. The functions complement each other; they do not progress at the same rate.

**Does this apply to non-fintech?**
The shape generalises. The specific stage-appropriateness shifts by industry — heavily-regulated industries (pharma, aerospace, banking) tend to operate at stage 3-4 by necessity; lightly-regulated industries (SaaS, consumer apps) often stay at stage 2 productively.

**Should the PMO Head report to the CEO?**
At stages 1-2, usually no — reporting to a COO, CPO, or VP Operations is more natural. At stages 3+, direct executive sponsorship is more important than the reporting line; the PMO can report to the COO and still have CEO sponsorship that gives it the standing it needs.

---

If this resonated, also read [Building a PMO From Scratch in Fintech](/blog/building-pmo-from-scratch-fintech), [Where PMOs Fail: Six Patterns](/blog/where-pmos-fail-six-patterns-fintech-programmes), and [The RAID + SteerCo PMO Stack That Ships](/blog/raid-steerco-pmo-stack-that-ships).`,"psd2-sca-exemptions-tra-low-value-recurring":`If you read only the PSD2 Regulatory Technical Standards on Strong Customer Authentication, the exemptions look mechanical: a checklist, five doors, walk through the right one and the issuer leaves the cardholder alone. The first time a senior payments PM tries to operationalise that checklist, the picture changes. The doors are open in writing and partly closed in practice. The issuer can override every exemption you flag, the scheme will count fraud across all your TRA traffic regardless of which acquirer triggered it, and the merchant cares about abandon-recovery, not exemption taxonomy.
...
**Recurring transactions.** Available for fixed-amount, same-merchant subscriptions where the first transaction was authenticated with SCA. The exemption applies only to subsequent transactions of the *same* amount; any change in the amount resets the cycle.
...
The single thing most acquirer-side product teams underestimate is that the exemption flag is a *request*, not a *grant*. The cardholder's issuing bank holds the decision. Five things the issuer can do regardless of what you flag:
...
The ceilings are *transaction value*, not *transaction count*. Two practical consequences:
...
**3. Recurring-amount drift.** A subscription that bumps the amount by 1% (currency rounding, FX float) breaks the recurring exemption. Suddenly the issuer step-ups. Recurring exemption logic must clamp to *exact* amounts in the original authorisation currency.
...
If this resonated, also read [EMV 3DS2 Step-Up Logic + Frictionless Flow Optimisation](/blog/emv-3ds2-step-up-frictionless-optimisation), [MDES + Network Tokenisation](/blog/mdes-network-tokenisation-how-it-actually-works), and [MPGS Architecture](/blog/mpges-mastercard-payment-gateway-services-architecture).`,"risk-adjusted-backlog-prioritisation-payments":`The standard PM prioritisation frameworks — RICE, ICE, WSJF, MoSCoW, Kano — all share a hidden assumption: shipping the wrong thing costs *roughly nothing*. You burn engineering time. You miss the quarter. You learn something. You move on.

The assumption is roughly true in SaaS. In payments it is structurally wrong. Shipping the wrong thing can cost the platform a scheme certification, an audit finding, a regulator sanction, a merchant lawsuit, a chargeback wave that breaches the band ceiling, a sev-1 incident that pulls executives away from the next quarter, or the licence itself.

The risk-adjusted backlog is the prioritisation discipline that takes the asymmetric downside seriously. It does not replace RICE; it sits on top of it. The senior payments PM who has been through one audit cycle has internalised it.

This is the operating definition: the five risk axes, the formula that does not pretend to be RICE, the four moves that earn the most lift, and the operating cadence that makes it stick.

## What "risk-adjusted" actually means

A standard prioritisation formula gives every backlog item a score on **upside only**: how much value will this produce if it ships well. RICE is the cleanest example: \`(Reach × Impact × Confidence) / Effort\`. Higher = better. The formula is symmetric about the value axis; downside enters only via the Confidence factor, which is a probability of *delivery*, not a probability of *harm*.

Risk-adjusted prioritisation adds the downside axis explicitly. Every backlog item gets two scores:

- **Value score.** What this delivers if it ships well. RICE / WSJF / whichever value-side formula the team uses.
- **Downside score.** What this costs if it ships badly. The cost when delivery still happens but the outcome is harm — to the licence, to merchants, to the audit posture, to the team's capacity.

The ranking is not "high value first" or "low risk first". It is "highest value-net-of-downside first", where the downside is real money in the same units as the upside.

## The five downside axes in payments

A working set of axes. Each scored 1–5; the highest axis dominates.

**1. Licence exposure.** What is the worst plausible regulator action if this ships badly? 5 = central-bank suspension, 4 = formal warning + capital implications, 3 = audit finding requiring remediation, 2 = expected supervisory comment, 1 = no licence touch.

**2. Scheme posture.** What is the worst plausible scheme response? 5 = certification loss, 4 = mandatory remediation programme, 3 = scheme review under chargeback monitoring, 2 = scheme-specified observation, 1 = no scheme touch.

**3. Customer / merchant impact.** What is the worst plausible customer outcome? 5 = stranded funds, 4 = portfolio-wide outage, 3 = merchant-tier service degradation, 2 = isolated merchant disruption, 1 = invisible to customers.

**4. Financial loss.** What is the worst plausible direct financial cost? 5 = capital-impacting (>1% of equity), 4 = material P&L impact, 3 = noticeable expense line, 2 = absorbed in run-rate, 1 = negligible.

**5. Reputational / contractual risk.** Worst plausible reputational consequence? 5 = published regulator action / press coverage, 4 = merchant churn at scale, 3 = senior contractual escalation, 2 = isolated relationship damage, 1 = invisible.

The downside score for an item is the *maximum* across the five axes, not the average. A single 5 on any axis dominates; that is the asymmetric-downside reality of payments work.

## The formula that does not pretend to be RICE

The standard RICE formula does not extend cleanly to downside. The risk-adjusted version is closer to expected-value:

\`Adjusted score = Value × Probability(good outcome) - Downside × Probability(bad outcome)\`

In practice, teams do not need a continuous probability. The senior PM uses a banded version:

| Downside score | Probability(bad outcome) | Multiplier on the downside |
|----------------|---------------------------|----------------------------|
| 1–2 | ~5% | Add as footnote, do not deduct |
| 3 | ~10–15% | Deduct 10–15% × downside cost |
| 4 | ~20–30% | Deduct 20–30% × downside cost |
| 5 | ~30–50% | Deduct 30–50% × downside cost |

Items with a downside-5 axis are not eliminated automatically. They are flagged for special treatment: more architectural review, more rollout staging, more pre-launch testing, and an explicit go/no-go gate that the CPO (and sometimes the CRO or General Counsel) signs.

The point of the formula is not the arithmetic. It is the *posture*. Items with high downside cannot be ranked next to items with low downside without acknowledging the asymmetry. The team that does not make the asymmetry visible defaults to "RICE rules" and ships the wrong things first.

## Four moves that earn the most lift

### Move 1 — Make the downside score visible in every prioritisation review

The simplest move. The backlog spreadsheet (or Linear / Jira / Productboard, whichever) has a "downside" column visible alongside RICE. Every item is scored in both. The ranking shows both scores. The conversation in the quarterly review explicitly tests trade-offs that ignoring downside would have hidden.

The behavioural effect is more important than the numerical effect. The team stops talking about "highest RICE" and starts talking about "highest value-net-of-downside".

### Move 2 — Build an explicit "regulator / scheme deadline" lane

Items driven by a regulator or scheme deadline get a separate lane on the board. The lane is not graded on RICE; it is graded on *delivery against the external date*. Mixing deadline-bound items with discretionary items on the same ranked list produces two failure modes: either the deadlines crowd out everything (organisational lockup) or RICE-ranked items push the deadlines off the slate (regulatory exposure).

The deadline lane has its own capacity (often 30–50% of total engineering capacity in regulated quarters), its own ownership, its own status reporting. The RICE-ranked lane covers the discretionary budget.

### Move 3 — Pre-mortem every downside-4-or-5 item

For every item with a downside score of 4 or 5, before the engineering work starts:

- The PM and the lead engineer write a one-page pre-mortem: what could go wrong, what would the impact be, what is the rollback path.
- The risk / compliance lead reviews and signs.
- The CPO (and sometimes CRO / GC) explicitly approves the launch criteria.

The pre-mortem is the cheap protection: 2–4 hours of structured pessimism that prevents 50+ hours of incident response. It also produces the rollback documentation that the engineering team will need if the launch goes wrong.

### Move 4 — Track the "did not ship because of downside" log

The discipline is incomplete until it is visible. The senior PM keeps a log of items that did *not* ship this quarter (or did not ship at all) because the downside math said no. The log is reviewed monthly by the leadership team.

Two functions:

- It makes the discipline auditable. "We did not ship X because" is a stronger conversation than "we deprioritised it".
- It builds the team's risk intuition over time. Junior PMs see why senior PMs made the calls they made; the rubric internalises faster.

## What this is not

The risk-adjusted backlog is *not* an excuse to avoid risk. Payments product work is, by its nature, high-stakes work. The discipline is to *price* the risk, not to avoid it.

Three patterns to watch for, where teams use risk-adjusted framing as a hiding place:

**1. Inflating downside on items the PM doesn't want to ship.** The downside score becomes a veto tool. The check: are downside scores stable across reviews, or do they move when the PM wants a different outcome? If they move, the scoring is being gamed.

**2. Using downside to slow regulator deadlines.** "We can't ship this in 3 months, the downside on rushed work is too high." Sometimes true; often a way to push the deadline. The regulator wins the conversation; downside framing does not change the calendar.

**3. Refusing to ship any downside-5 item.** Some downside-5 items have to ship — they are mandatory. The discipline is to ship them with extra protection, not to refuse.

The senior PM uses the framework to make better calls, not to delay them.

## The operating cadence

A working rhythm for keeping the framework alive:

- **Quarterly planning.** Every backlog item is scored on value and downside. The slate is built with explicit risk-budget allocation between deadline-bound, RICE-ranked, and downside-flagged work.
- **Monthly review.** Re-score items where the world has changed (regulator publishes, scheme updates rules, a major merchant escalates). Adjust the lane capacities if the deadline pressure has shifted.
- **Pre-launch gate.** Every item with downside 4 or 5 has its pre-mortem reviewed and signed. No exception.
- **Post-launch post-mortem.** For any item that produced an unexpected incident: was the downside score correct? Update the scoring rubric for future items.
- **Quarterly retrospective.** Review the "did not ship because of downside" log. Were the calls right? What got better; what got worse.

The rituals matter. Frameworks that exist only on paper degrade in 60 days. Frameworks that are part of the team's regular cadence outlast the PM who introduced them.

## The senior-PM tell

The interview question that distinguishes senior payments PMs on this: "your RICE-top item has a downside-5 axis on regulator exposure. The board is pushing to ship it. What do you do?"

The junior answer accepts the board's call. The senior answer does not refuse, but pushes back through the discipline: explicit pre-mortem; explicit rollout staging; explicit gate criteria from the CRO and the CPO before launch; explicit kill criteria during rollout. The item ships, but it ships with the downside priced. If the rollout shows the downside materialising, the senior PM kills it without apology, because the kill criteria were agreed upfront.

The conversation is not "should we ship this fast" vs "should we delay this". It is "what does shipping this responsibly look like, and what are the costs we accept if responsibility means a 30% chance of a 4-week delay". The senior PM owns the framing; the board owns the call.

## FAQ

**How is this different from WSJF?**
WSJF (Weighted Shortest Job First) handles cost-of-delay well, which maps to regulator deadlines. WSJF does not natively handle the asymmetric downside of payments work (failure modes that cost the licence). A WSJF + downside-axis hybrid is roughly what we are describing.

**How is this different from Kano-style "must-haves"?**
Kano sorts features by user preference. The downside axes here are not about user preference; they are about systemic risk. A licence-protecting feature is not a Kano must-have because users may not even see it.

**Does this make the backlog slower?**
Marginally. The extra scoring takes ~10 minutes per item; the pre-mortem on downside-4-or-5 items takes 2–4 hours. The throughput is comparable; the *direction* of throughput is different. Items that would have produced incidents are now caught at planning time.

**What if the team disagrees on downside scores?**
Productive disagreement. The senior PM facilitates a calibration conversation; the goal is not consensus but visibility. Disagreement on downside often reveals that the team has different mental models of what the platform can absorb.

**Does this map to non-regulated products?**
Partially. Any product where a failure produces step-function harm (financial services, healthcare, infrastructure, safety) benefits from the asymmetric framing. Products where failure is "we learn and iterate" do not need it.

**Should engineering also score downside?**
Yes — engineering's view often catches risk axes that product misses (system-stability risk, dependency risk, technical-debt accumulation). The senior PM facilitates the joint scoring; the score is one number, but two reviewers calibrate it.

---

If this resonated, also read [CSPO + RICE in Practice](/blog/cspo-rice-payments-roadmap-walkthrough), [Product Management for Payments Platforms](/blog/product-management-for-payments-platforms), and [Payments PRD Template](/blog/payments-prd-template-nine-sections-senior-pm).`,"scheme-settlement-t-plus-1-t-plus-0-real-time-working-capital":`Every PM working on a card acquirer eventually has a meeting with a merchant who wants "same-day settlement". The conversation is usually friendly until the PM tries to explain why the platform settles on T+1, the merchant points to a competitor who advertises T+0, and the PM realises they have never written down what settlement timing actually costs.
...
A card authorisation is not money movement. It is a *promise*: the issuer holds funds against the cardholder's available limit. The actual money does not move on auth day.
...
**T+0 / same-day settlement.** The merchant sees funds available on the *same calendar day* as the transaction. Two distinct mechanisms produce same-day funding:

- *Scheme-supported same-day.* Some schemes have introduced same-day settlement windows in certain regions. The scheme runs a second daily cycle, and the acquirer can present transactions for the earlier cut-over.
- *Acquirer-funded same-day.* The acquirer pays the merchant out of its own balance sheet (an advance) before the scheme has settled. The acquirer carries the float between funding the merchant and receiving the scheme settlement.
...
The headline difference between T+1 (scheme-paced) and same-day/real-time (acquirer-funded) is **who carries the float**. T+1 is a money-movement timing. Same-day and real-time are *credit products* dressed as money-movement timing — the acquirer is lending the merchant money against future scheme settlement.
...
For a merchant generating $500M TPV, the working-capital cost of moving from T+1 to T+0 is in the $80–100k range *annually*, plus the intraday liquidity reserve the acquirer has to maintain to absorb spikes.
...
Even merchants on the standard cadence sometimes see settlement held — funds withheld for fraud review, AML hold, account dispute, or a sudden chargeback spike. The senior PM owns the *hold-and-release* policy:
...
Related: [Reconciliation Is Product Infrastructure](/blog/reconciliation-is-product-infrastructure) · [Exception Management](/blog/exception-management-reconciliation) · [Case study: Reconciliation & Ledger Controls](/product-work/settlement-reconciliation)`,"vendor-governance-pmo-highest-leverage-surface":`Most fintech PMOs invest in delivery governance — RAID logs, SteerCo cadence, programme reporting — and leave vendor governance as a procurement adjunct that someone else owns. The pattern is structurally wrong. In a fintech of any meaningful scale, the vendor estate is the *largest* operational surface, the *fastest-changing* risk surface, and the *most regulator-visible* one. The PMO that owns it well changes the company's posture more than the PMO that ships another quarterly SteerCo refresh.

This is the operator's view: the five vendor-governance disciplines, the SLAs that actually matter (not the ones the vendor proposes), the split between what the PMO owns and what procurement owns, and the four failure modes that produce regulator findings.

## Why vendor governance is the highest-leverage PMO surface

Three structural reasons.

**1. The vendor estate touches every regulator-facing operation.** Sanctions screening, KYB, fraud detection, document verification, ID&V, dispute management, settlement, treasury — all are vendor-supplied or vendor-augmented in most fintechs. A regulator inquiry on any of these is, in practice, an inquiry on the vendor's controls. The PMO that has not catalogued the vendor estate against the regulator-facing operations is the PMO that produces a 2-week scramble per inquiry.

**2. Vendor relationships are the largest concentration of "trust we cannot audit ourselves" in the company.** The fintech relies on the vendor to do what the contract says; verification is mostly attestation-based. Without explicit governance (performance monitoring, SLA tracking, periodic re-evaluation), the trust degrades silently until something breaks publicly.

**3. Vendor lifecycle events are change-management surfaces.** Vendor onboarding, SLA changes, contract renewals, vendor exits — each is a programme-shaped event with cross-functional touchpoints. The PMO that runs these as programmes (rather than as procurement transactions) saves quarters of remediation work.

The leverage is real and consistently under-invested. A 5-person vendor-governance function in a 300-person fintech routinely produces the equivalent of 30+ FTE of risk-reduction value per year.

## The five disciplines

A working set of disciplines. Each one is a PMO surface; together they form the vendor-governance operating model.

### Discipline 1 — Vendor onboarding

**What it covers.** The end-to-end process from "we want to use vendor X" to "vendor X is integrated into our operations" — including risk assessment, due diligence, contract negotiation, technical integration, and operational readiness.

**What the PMO owns.** The process itself. The risk assessment criteria. The cross-functional review (legal, compliance, security, operations, finance). The go/no-go decision gates. The operational-readiness checklist that the vendor's first production traffic depends on.

**What good looks like.** A new vendor cannot start production traffic without a documented risk assessment, a compliance sign-off, a security review, a defined SLA with measurement methodology, and an exit plan. The onboarding cycle for a critical vendor is 8-14 weeks; for a low-risk peripheral vendor, 2-4 weeks. The process is documented; deviations require sign-off.

**What bad looks like.** A vendor starts traffic because a product team wired up the integration. The risk assessment was a checkbox in procurement's CRM. The exit plan does not exist.

### Discipline 2 — Vendor performance management

**What it covers.** Ongoing tracking of vendor performance against the agreed SLAs, the production health of the vendor relationship, and the relationship at an operational level.

**What the PMO owns.** The SLA framework. The performance reporting cadence (monthly per critical vendor, quarterly per important vendor, annually per peripheral). The escalation path when SLA is missed.

**What good looks like.** Every critical vendor has a documented set of SLA metrics tied to its operational role. Performance is measured by the platform's own instrumentation (not the vendor's self-report). Misses produce documented findings; recurring misses produce contract-renegotiation triggers.

**What bad looks like.** SLAs exist in the contract but are not measured. Vendor self-reports performance. Misses are noted in side conversations and forgotten by the next quarter.

### Discipline 3 — Vendor risk monitoring

**What it covers.** The risk profile of each vendor — financial health, security posture, regulatory exposure, concentration risk to the fintech.

**What the PMO owns.** The risk-monitoring framework. The cadence of re-assessment (annually for most; semi-annually for critical; on-event for adverse news). The escalation when a vendor's risk profile changes materially.

**What good looks like.** Every vendor has a risk tier. Each tier has a re-assessment cadence. Adverse-news monitoring covers the vendor and its corporate group. Material risk changes trigger a documented escalation to the executive team.

**What bad looks like.** Vendor risk is assessed at onboarding and never re-assessed. The vendor that was financially healthy three years ago and is now in financial distress is still rated low-risk in the PMO's spreadsheet.

### Discipline 4 — Contract lifecycle management

**What it covers.** The contract itself — its terms, its renewal cycle, its amendment history, its alignment to the operational reality.

**What the PMO owns.** The contract repository. The renewal calendar (with 6-month, 3-month, 1-month notice triggers). The change-management process for contract amendments. The audit trail of contract changes.

**What good looks like.** No critical vendor contract renews silently. Every renewal triggers a re-assessment of fit, SLA, pricing, and exit terms. Amendments are reviewed by legal + the operational owner; both must sign.

**What bad looks like.** Contracts auto-renew on terms agreed three years ago for an operational profile that has changed completely. The contract repository is a SharePoint folder nobody owns.

### Discipline 5 — Vendor exit planning

**What it covers.** What happens when a vendor relationship ends — voluntarily, contractually, or under duress.

**What the PMO owns.** The exit plan for every critical vendor (drafted at onboarding, updated annually). The data-portability provisions. The runbook for forced exit (vendor failure, regulator-mandated exit, security incident).

**What good looks like.** Every critical vendor has an exit plan that names the replacement option, the data extraction path, the operational continuity plan during transition, and the realistic timeline (typically 6-12 months for a critical integration).

**What bad looks like.** No exit plan exists. The first thought of "what if vendor X disappears tomorrow" happens after vendor X has announced bankruptcy.

## The SLAs that actually matter

Most vendor SLAs that get into contracts are *vendor-proposed*. They tend to be the metrics the vendor is comfortable hitting. The metrics the operator needs are usually different.

A working set of SLA categories the PMO should insist on:

**1. Availability.** Uptime expressed in a measurable way (5-min interval averages, not yearly averages). 99.9% averaged annually allows for 9 hours of yearly downtime in any single window; 99.9% measured per-hour allows for materially less. Spell out the measurement.

**2. Response time.** P50 / P95 / P99 latency on the vendor's response, measured by the platform's own instrumentation. The vendor's measurement is suspect by default; the platform's measurement is the contract.

**3. Accuracy / quality.** For decision-supplying vendors (sanctions screening, fraud scoring, ID&V), the accuracy of the vendor's output measured against ground truth. The SLA is usually expressed as false-positive rate / false-negative rate / coverage rate.

**4. Incident response.** Time to acknowledge an incident, time to engage senior support, time to provide a resolution path. Measurable; usually missing from vendor-proposed contracts.

**5. Data handling.** Time to data deletion on request; data residency; data-export format and timeline. Increasingly regulator-relevant.

**6. Change notice.** Notice before vendor changes their API, their controls, their pricing. The platform needs at least 60-90 days for material changes; the vendor's proposal often gives 30.

**7. Audit access.** The platform's right to audit the vendor's controls (directly or via independent assessor). Standard in regulated-services contracts; often missing in tech-vendor contracts.

The discipline is to insist on platform-measured SLAs, not vendor-reported ones. Self-reporting from the vendor is a relationship artefact, not a contract instrument.

## What the PMO owns vs. what procurement owns

A working split:

| Function | PMO | Procurement |
|----------|-----|-------------|
| Initial vendor identification | Operational team | Procurement runs the RFP |
| Risk assessment criteria | PMO defines | PMO + risk function applies |
| Commercial negotiation | Procurement leads | PMO supports on SLA + exit terms |
| Contract signing | Legal + procurement | PMO is signatory on operational schedules |
| Ongoing performance management | PMO owns | Procurement supports on renewals |
| Risk re-assessment | PMO + risk | Procurement supports on terms |
| Exit execution | PMO leads | Procurement supports on termination terms |

The split is sometimes confused: in some organisations, procurement owns the vendor entirely; in others, the PMO owns the vendor entirely. Both extremes produce gaps. Procurement-only governance leaves the operational reality unmanaged; PMO-only governance leaves the commercial reality un-leveraged.

The clean split: procurement owns the *commercial* and *contract* surface; PMO owns the *operational* and *risk* surface; both engage on the *renewal* and *exit* surfaces.

## The four failure modes that produce regulator findings

**1. The "we have a contract, therefore we have governance" trap.** A contract is a permission; governance is the operating discipline that makes the contract work. Regulators see through this immediately — they ask for performance evidence, not contract terms.

**2. The "vendor self-reports everything" trap.** Compliance functions sign off on vendor relationships partly on the basis of vendor attestation. Attestation is a starting point; independent measurement is the test.

**3. The "concentration risk we never measured" trap.** The fintech's three biggest critical vendors share a parent company, or rely on the same upstream provider. When the upstream provider has an incident, three of the fintech's critical capabilities go down simultaneously. The PMO that has not mapped the vendor estate's concentration risk discovers this in the post-mortem.

**4. The "exit plan that does not survive contact" trap.** The exit plan was drafted at onboarding and never tested. When the vendor announces a price increase that triggers exit, the platform discovers that the data-portability provisions are unenforceable, the replacement vendor needs 18 months to onboard, and the existing contract has a 12-month exit lock-in.

## What a vendor-governance function actually looks like

A working set of deliverables for a 4-6-person vendor-governance function in a 300-1,500-person fintech:

- **Vendor inventory.** Every vendor catalogued with: tier, owner, regulator-facing role, contract value, contract renewal date, last risk assessment.
- **SLA dashboard.** Critical vendors monitored continuously; non-critical reviewed monthly. Misses logged and tracked to remediation.
- **Risk-monitoring pipeline.** Adverse-news monitoring across the vendor estate; quarterly risk re-tiering reviews; annual full re-assessment for top tier.
- **Contract calendar.** Visible 18 months out; renewal triggers automated 6/3/1 months ahead of expiry.
- **Exit-plan library.** Every critical vendor has one; tested annually via a tabletop exercise.
- **Regulator-facing pack.** The vendor governance artefacts that close a regulator inquiry in under 48 hours.

The investment is real (4-6 people, supporting tooling, ongoing engineering integration into the platform's monitoring); the return is structurally large (the avoided incidents, the closed regulator findings, the leverage in vendor renegotiations).

## The senior PMO tell

The interview question that distinguishes senior PMO leaders on vendor governance: "your highest-value vendor announces a 40% price increase at renewal. The contract has a 30-day notice. What do you do?"

The junior answer talks about negotiating. The senior answer reads: pull the vendor's performance data for the last 18 months (we should have it); pull the alternative vendors that have come on the radar (we should know who they are); estimate the realistic switching cost (we should have the exit plan); know whether the vendor's threat is genuine or posturing (we should have the relationship); engage procurement on the negotiation with this evidence pack ready in a week. If we have done none of this preparation, the vendor wins the negotiation, and we book the 40% as a cost discipline failure.

That preparation is what vendor governance buys. The 40% price increase is when the function pays back the cost of building it.

## FAQ

**Should every vendor be governed?**
No. The lowest-tier vendors (a SaaS tool with 5 users that touches no operational data) get light-touch governance — registration, basic risk screen, periodic re-check. Heavy governance is reserved for vendors that touch the regulator-facing surface or carry material concentration risk.

**Is "vendor governance" the same as "third-party risk management"?**
Overlapping but not identical. TPRM is the risk-management lens (data security, financial health, regulatory posture). Vendor governance is broader — adds performance management, contract lifecycle, operational integration. Most regulators expect both.

**Who is the senior owner — PMO, COO, or CRO?**
Varies. At smaller scale, COO or CRO. At medium scale, a dedicated Head of Vendor Governance reporting to COO or CRO. At very large scale, the function may sit under a CRO with delivery support from PMO. The pattern matters less than the discipline being owned somewhere with executive teeth.

**How does this relate to PMO maturity?**
Vendor governance is one of the disciplines that distinguishes a stage-3 PMO from a stage-2 PMO. Many fintechs reach stage 2 PMO maturity without building this function; reaching stage 3 typically requires it.

**Does this apply to non-vendor partners (banking partners, scheme partners)?**
The discipline generalises. Banking partners and scheme partners are a different governance category (they are counterparties, not vendors), but the surfaces — performance management, risk monitoring, contract lifecycle, exit planning — all apply.

**Does AI / LLM vendor procurement need different governance?**
Yes — three additions on top of standard vendor governance: model-version pinning, data-handling provisions covering training-data use, and a defined model-change-notice requirement. The base vendor-governance discipline carries over; the additions are AI-specific.

---

If this resonated, also read [Building a PMO From Scratch in Fintech](/blog/building-pmo-from-scratch-fintech), [PMO Maturity Model: 5 Stages](/blog/pmo-maturity-model-five-stages), and [The RAID + SteerCo PMO Stack That Ships](/blog/raid-steerco-pmo-stack-that-ships).`,"what-is-core-banking-system-when-to-replace":`Most fintech operators inherit their core banking decision rather than make it. The core was chosen by the founder, or by the previous CTO, or by the bank's IT director in 2009. The decision shapes the next decade of the company's product velocity, regulatory posture, and operating cost. By the time the senior operator looks closely at it, the decision has compounded.
...
A core banking system, at the simplest definition, holds the *book of record* for the bank's customer accounts and the *engine* that processes the transactions that move money into and out of those accounts. Four core functions:
...
The reason brownfield timelines balloon is not the technology — it is the *operational complexity* of moving a live book of business from one system to another while the bank continues to operate. Every operator who has done a brownfield replacement has the same observation: the technology was 30% of the work; the everything-else was 70%.
...
**Architecture.**
4. Real-time processing — true real-time, or batch with a real-time façade?
5. API surface — what's available, what's reliable, what's missing?
6. Multi-currency and multi-entity — does it handle your geographic footprint?

**Operations.**
7. Hosting model — SaaS, dedicated, on-prem, hybrid?
8. Disaster recovery — what's the RPO/RTO? Tested how?
9. Operational reliability — what is the realistic uptime track record from existing customers?

**Vendor relationship.**
10. Roadmap alignment — is the vendor going in the direction you need?
11. Customer-influence model — can you influence roadmap, or are you on the receiving end?
12. Exit clauses — what does data extraction look like if you ever leave?
...
**The root cause.** A feature that was available at training time was implicitly using information that would not be available at inference time. The classic example: a "transaction was reviewed by the fraud team" feature that is only set *after* the review. The model effectively cheated.

**The payments specifics.** Label leakage is endemic in payments because the labels (fraud / not fraud) arrive *days to weeks* after the transaction, the data warehouses are built for analytics not for ML training, and the feature pipelines often include flags set during the dispute lifecycle.
...
**The root cause.** AUC is a rank-ordering metric. It says "the model can distinguish fraud from non-fraud", but it does not say what the *operating point* should be. At the operating point chosen for deployment, the model traded off precision and recall in a way that did not match the business cost. A model with 0.91 AUC can absolutely have worse production performance than a model with 0.86 AUC if the operating point on the first model is wrong.
