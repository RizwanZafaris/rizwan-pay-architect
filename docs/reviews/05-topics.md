...
**Hub CTA wording is inconsistent with the page's purpose.** "Filter essays →" suggests you're going back to `/blog` with a filter applied. If that's the actual behaviour, fine — but the page title says "Eight _hubs_", which implies a destination page per topic. Decide:
...
**The 8-hub grid is symmetric and reads as templated.** Add a one-line POV under each hub title so visitors know _your_ angle, not just the topic:
...
<meta
  name="description"
  content="Field notes on cross-border payments infrastructure: corridor design, FX, settlement, regulatory mapping. 17 essays and 2 case studies from production deployments in MENA and South Asia."
/>
```
