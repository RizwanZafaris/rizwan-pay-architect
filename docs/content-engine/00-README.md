...
| **Google Analytics (GA4)** | Direct GA4 gtag pipeline is live with booking-created events; GTM is retired. Closes the loop: which essay → which recruiter page → created booking. | Content-to-conversion attribution table, monthly | Tells us which clusters produce calls, not just traffic | **Critical** |
