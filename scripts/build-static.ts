...
async function writeNotFoundPage(worker: Worker) {
  ...
  html = stripDevServerScripts(html);
  html = addTrailingSlashes(html);
  // The route's useEffect title never runs on the JS-less export, so the 404
  // page would otherwise ship the generic site title.
  html = html.replace(
    /<title>[^<]*<\/title>/i,
    "<title>404 — Page Not Found | Rizwan Zafar</title>",
  );
